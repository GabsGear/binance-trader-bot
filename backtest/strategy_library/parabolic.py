...
class Strategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.rsi = bt.indicators.RSI_EMA(self.datas[0])
        self.resistance = bt.indicators.Highest(self.datas[0].close, period=2)
    ...
    def next(self):
        ...
        TOP_CLOUD = self.dataclose[0] > yellow and self.dataclose[0] > blue
        BUY_SIGNAL = self.psar[0] < self.dataclose[0] and LAST_PSAR > LAST_CLOSE and LAST_PSAR > self.psar[0] and self.pgo[0] > 0 and TOP_CLOUD == False
        #SELL_SIGNAL = self.psar[0] > self.dataclose[0] and LAST_PSAR < LAST_CLOSE and LAST_PSAR < self.psar[0]
        SELL_SIGNAL = self.resistance[0] >= self.datas[0].high[0]
        ...
        LAST_PSAR = self.psar[0]
        LAST_CLOSE = self.dataclose[0]
