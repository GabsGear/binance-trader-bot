...
def media_movel(bot_config):
	try:
		data = getDataDecision(bot_config)
		size = len(data['c'])
		last_c = data['c'][size-2:size-1] # Last close price
		close = data['c'][size-31:size-1] # 30 Close prices
		mean = tb.SMA(np.array(close, dtype=float), timeperiod=30)
		print("[+] ALVO COMPRA: %.8f | LAST CLOSE: %.8f" % (mean[29], last_c[0]))
		if(last_c[0] < mean[29]):
			return 'buy'
	except:
		return 'none'
...
def map(bot_config):
	map = {
		...
		7: media_movel(bot_config), #MEDIA MOVEL
	}
	#print map
	return map
