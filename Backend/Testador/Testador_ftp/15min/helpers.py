...
class Helpers:
    ...
    def writeOutput(self, bot_id, data):
	    file = open('/home/backtest/15min/DataPlot/'+str(bot_id)+'-output.txt', 'a+')
	    file.write('['+str(self.time_now())+'] '+data+"\n")

    #write simple log
    def writeOutputTest(self, strategie,coin , time, buyValue, sellValue):
        file = open('/home/backtest/15min/Rank/'+str(strategie)+'-'+ str(coin)+'-Test-output.txt', 'a+')
        file.write('['+ str(time) +']' + ',' + str(buyValue) + ',' + str(sellValue)+"\n")
        #file.close()

    def insertRank(self, strategy, coin, profit,acerto):
        file = open('/home/backtest/15min/Rank/StrategyRankProfit.txt', 'a+')
    ...
    def generateFinalLog(self, strategy, log, coin, begin, end, ncandles):
        ...
        file = open('/home/backtest/15min/FinalLog/'+str(strategy)+'-'+ str(coin)+'-Final-output.txt', 'a+')
