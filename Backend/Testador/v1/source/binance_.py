...
class Binance_opr:
    def getSymbolList(self):
        ...
        for x in range (0, 298):
            print (x)
            symbol = client.get_exchange_info()["symbols"][x]["symbol"]
            
            if (symbol[len(symbol)-3:len(symbol)] == 'BTC' or symbol[len(symbol)-4:len(symbol)] == 'USDT'):
                print(symbol)
    ...
    def getStoricalCandles(self, coin, day):
        ...
        klines = client.get_historical_klines(coin, Client.KLINE_INTERVAL_30MINUTE, "1 Jan, 2017", "1 Mar, 2018")
